...
// Componente al que voy a redirigir después del login
import Reservas from "./Reservas"

// CREAMOS NUESTRA FUNCION LOGIN QUE RETORNARA UN FORMULARIO Y DENTRO DE EL
// LE PONDREMOS LA LOGICA PARA EL LOGIN
function Login() {
  // Estado para guardar lo que el usuario escribe en el input de nombre
  const [nombre, setNombre] = useState(""); // ESTA VACIO
  // Estado para la contraseña
  const [contrasena, setContrasena] = useState(""); // ESTA VACIO
   // Estado para validar si hay campos vacíos
  const [campoVacio, setCampoVacio] = useState(false); // ESTA EN FALSE
  // Estado para controlar si pasamos a la siguiente vista
  const [irAReservas, setIrAReservas] = useState(false); // ESTA EN FALSE
  ...
  // le retorno el componente reservas
  if (irAReservas){
